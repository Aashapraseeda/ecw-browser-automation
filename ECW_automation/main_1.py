...
def is_demo_patient(visit_reason):
    ...
    if vr == "test":
        return True
    if re.match(r"^\d+\s*year\s*test$", vr):
        return True
    return False
...
def read_patients_from_excel():
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        visit_reason = row[col["Visit Reason"]]
        ...
        if not form_name:
            print(f"Skipping {last_name} {first_name} - no ASQ form for visit type: {visit_type_desc}")
            continue
        patients.append({
    ...
    print(f"Filtered schedule ({len(filtered_rows)} demo patients) saved to: {FILTERED_EXCEL_PATH}")
    return patients
...
async def click_calendar_option(iframe, day_str, descriptions_to_try, label):
    """Try clicking calendar option with multiple description variants then fallback to no description."""
...
async def ecw_export_schedule():
    print("\n" + "="*50)
    print("STEP 0 — ECW: EXPORTING SCHEDULE (3-DAY WINDOW)")
    print("="*50)

    # Both 2-letter and 3-letter variants — try both
    ...
    async with async_playwright() as p:
        ...
        # --- WAIT FOR REPORT TO GENERATE (2-3 min) ---
...
async def check_and_download_completed(page, patients):
    """
    Search each patient without a status filter, then check whether they have
    a Completed submission. Patients whose form was sent more than once will
    have multiple Export PDF buttons on their detail page - match the one
    whose row mentions the CURRENT visit's form name so a stale/unrelated
    completed submission isn't downloaded and mislabeled under today's filename.

    Single pass only - returns the list of patients newly downloaded this
    run. Cron re-invoking this script provides the "check again later"
    behavior; there is no internal wait loop here anymore.
    """
    ...
    for patient in patients:
        try:
            ...
            # If this account has multiple completed submissions (form sent more
            # than once), match the one for the CURRENT visit's form instead of
            # blindly taking the first - the first one is not guaranteed to be
            # the one for today's visit type.
            target_index = 0
            if export_count > 1:
                print(f"Note: {patient['acct_no']} has {export_count} completed submissions "
                      f"(form likely sent more than once)")
                matched = False
                for i in range(export_count):
                    row = export_buttons.nth(i).locator("xpath=ancestor::*[self::tr or self::li or self::div][1]")
                    try:
                        row_text = (await row.inner_text(timeout=2000)).lower()
                    except:
                        continue
                    if patient["form_name"].lower() in row_text:
                        target_index = i
                        matched = True
                        break
                if matched:
                    print(f"Matched submission for form '{patient['form_name']}' at position {target_index + 1}")
                else:
                    print(f"Could not match a submission to '{patient['form_name']}' - "
                          f"defaulting to the first (most recent) one")
            ...
            print(f"Downloading completed form for {patient['acct_no']}...")
            async with page.expect_download() as download_info:
                await export_buttons.nth(target_index).click()
            download = await download_info.value
...
async def main():
    ...
    if pending:
        await pediforms_check_and_download(pending)
...
#                         f"{patient['last_name'].upper()}, {patient['first_name'].upper()}"
...
#     # ─── STEP 3: PEDIFORMS - DOWNLOAD FORMS ───
#     print("\n" + "="*50)
#     print("STEP 3 — PEDIFORMS: DOWNLOADING FORMS")
#     print("="*50)
...
#         # Use exported for demo — use completed for production
#         await page.get_by_role("combobox").nth(1).select_option("exported")
#         await page.wait_for_timeout(1000)
#         print("Status filter set to exported!")
...
#         for patient in patients:
#             try:
#                 print(f"\nDownloading for {patient['acct_no']} ({patient['last_name']} {patient['first_name']})")
...
#                 view_count = await page.get_by_role("link", name="View").count()
#                 if view_count == 0:
#                     print(f"No forms found for {patient['acct_no']} - skipping")
#                     await search_box.fill("")
#                     continue
...
#         for index, patient in enumerate(patients):
#             print(f"\nUploading for {index+1}/{len(patients)}: {patient['last_name']} {patient['first_name']}")
